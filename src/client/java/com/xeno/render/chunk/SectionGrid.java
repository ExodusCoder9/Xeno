...
import com.xeno.render.chunk.util.CircularIntList;
...
public class SectionGrid {
   ...
   public void repositionCamera(double x, double z) {
      ...
      for (int var31 = secX - (this.gridWidth >> 1) + xRangeStart; xRangeIterator.hasNext(); var31++) {
         int xRelativeIndex = xRangeIterator.next();
         int x1 = var31 << 4;
         zIterator.restart();

         for (int var33 = secZ - (this.gridWidth >> 1); zIterator.hasNext(); var33++) {
      ...
      for (int var32 = secX - (this.gridWidth >> 1) + xComplStart; xComplIterator.hasNext(); var32++) {
         int xRelativeIndex = xComplIterator.next();
         int x1 = var32 << 4;
         zRangeIterator.restart();

         for (int var34 = secZ - (this.gridWidth >> 1) + zRangeStart; zRangeIterator.hasNext(); var34++) {
   ...
   private void moveSection(
      int xRelativeIndex, int yRel, int zRelativeIndex, int x1, int z1, CircularIntList xList, CircularIntList zList, int xCurrentIdx, int zCurrentIdx
   ) {
      ...
      renderSection.inAreaIndex = (short)(
         (x1 - chunkArea.position.x() >> 4) + ((z1 - chunkArea.position.z() >> 4) * 8 + (y1 - chunkArea.position.y() >> 4)) * 8
      );
   }
   ...
   private void setNeighbours(RenderSection section, CircularIntList xList, CircularIntList zList, int xIdx, int zIdx, int x, int y, int z) {
      ...
      if (eastX != -1) {
         RenderSection neighbour = this.sections[this.getChunkIndex(eastX, y, z)];
         section.setAdjacent(neighbour, 5);
      }

      if (westX != -1) {
         RenderSection neighbour = this.sections[this.getChunkIndex(westX, y, z)];
         section.setAdjacent(neighbour, 4);
      }

      if (northZ != -1) {
         RenderSection neighbour = this.sections[this.getChunkIndex(x, y, northZ)];
         section.setAdjacent(neighbour, 2);
      }

      if (southZ != -1) {
         RenderSection neighbour = this.sections[this.getChunkIndex(x, y, southZ)];
   ...
   private void setYNeighbours(RenderSection section, int x, int y, int z) {
      if (y != this.gridHeight - 1) {
         RenderSection neighbour = this.sections[this.getChunkIndex(x, y + 1, z)];
